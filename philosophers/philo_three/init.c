...
/*   Updated: 2020/12/20 20:06:02 by qmarowak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!(\
	(data->philo_id = (pid_t*)malloc(sizeof(pid_t) * data->count_philo))
	...
	|| (data->philo_fork = sem_open("forks", O_CREAT, 0700, data->count_philo))
		== SEM_FAILED ||
	(data->death = sem_open("death", O_CREAT, 0700, 1)) == SEM_FAILED ||
	(data->swaiter = sem_open("swaiter", O_CREAT, 0700, data->count_philo))
		== SEM_FAILED)
	{
